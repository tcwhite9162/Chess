...
def render_check(window, row, col, color=(255, 0, 0)):
    ...
    y = BOARD_ORIGIN_Y + row * SQUARE_SIZE
    pygame.draw.rect(
        window, color, (x, y, SQUARE_SIZE, SQUARE_SIZE), width=CHECK_BORDER_WIDTH
    )
...
def render_captures(window, captures, color):
    ...
    for row, col in captures:
        ...
        pygame.draw.circle(
            window, color, (x, y), SQUARE_SIZE // 2, width=CAPTURE_RING_WIDTH
        )
...
def render_gameover(window, board, gamestate):
    ...
    pygame.draw.rect(
        overlay, OVERLAY_COLOR, (0, 0, overlay_width, overlay_height), border_radius=15
    )
    ...
    text_rect = text_surface.get_rect(
        center=(overlay_x + overlay_width // 2, overlay_y + overlay_height // 2)
    )
    ...
    for i, color in enumerate(["b", "w"]):
        ...
        if gamestate.turn == color and gamestate.clock.clock_started:
            pygame.draw.rect(
                window,
                outline_color,
                (x - 4, y - 4, clock_width + 8, clock_height + 8),
                border_radius=10,
            )
        else:
            pygame.draw.rect(
        ...
        pygame.draw.rect(
            window, background, (x, y, clock_width, clock_height), border_radius=10
        )

        surface = font.render(time_str, True, text_color)
        text_rect = surface.get_rect(
            center=(x + clock_width // 2, y + clock_height // 2)
        )
