...
def main():
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if gamestate.awaiting_promotion:
                    ...
                    if chosen_type:
                        gamestate.complete_promotion(board, chosen_type)
        ...
        if gamestate.clock.clock_started and current_time - last_clock_update >= 1000 and not gamestate.status['is_checkmate']:
            gamestate.clock.update()
            render_clock(window, gamestate)
